...
def compute_synsets_from_wordnets(old_wordnet, new_wordnet, pos):
    ...
    return extract_senses(reference)
...
def main():
    ...
    wn2 = WordNetCorpusReader('D:\\dialogue2020\\semeval-2016-task-14\\WN2.0', None)
    ...
    ft.vectorize_groups(noun_synsets, os.path.join(vector_path, "nouns_wordnet_fasttext.txt"), to_upper=False)
    ft.vectorize_groups(verb_synsets, os.path.join(vector_path, "verbs_wordnet_fasttext.txt"), to_upper=False)

    # vectorize words
    process_data(ft, os.path.join(input_path, "no_labels_nouns_en_new.tsv"),
                 os.path.join(vector_path, "nouns_fasttext.txt"))
    process_data(ft, os.path.join(input_path, "no_labels_verbs_en_new.tsv"),
                 os.path.join(vector_path, "verbs_fasttext.txt"))
